...
import com.kvw.jsonplaceholder.ui.userdetail.UserDetailFragmentDirections
    ...
    private fun setUpRecyclerView() {
        userListViewModel.albums.observe(this.viewLifecycleOwner, Observer { albums ->
                recyclerView_userDetail_albumList.swapAdapter(
                    UserDetailAlbumListAdapter(albums) {
                        findNavController().navigate(
                            UserDetailFragmentDirections
                                .actionUserDetailFragmentToAlbumDetailFragment(it)
                        )
                    }, true)
            }
        )
